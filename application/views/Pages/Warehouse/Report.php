	...
	<div class="content">
		...
		<div class="panel panel-flat">
			<div class="panel-heading">
				<h5 class="panel-title"><a href="<?=base_url()?>Warehouse/Print" class="btn bg-green-400 btn-sm" style="border-radius: 0px;" target="_blank">Print Ordered Report</a></h5>
			</div>
				...
				<table class="table datatable-basic table-striped">
					...
					<tbody>
						...
						<tr>
							...
							<td><?=$rows3['item']?></td>
							<td><?=$rows['quantity'].' '.$rows3['unit']?></td>
							<td><?=$row->order_status == 3 ? 'Verified' : 'Denied'?></td>
							<td><?=date('F d, Y - h:i A',strtotime($row->order_date))?></td>
						</tr>
						<?php } ?>
					</tbody>
				</table>
